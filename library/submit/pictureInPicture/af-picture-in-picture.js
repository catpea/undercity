// library/submit/pictureInPicture/af-picture-in-picture.js
//
// <af-picture-in-picture>
//
// Purpose-built submit component for the picture-in-picture processor.
...
//   preview-key  — inventory key for preview.avif/.mp4; blank = don't store
...
const JOB_TYPE = 'picture-in-picture';

// Files this processor writes and how to store them.
// fallback: if the primary filename is missing, try this instead (e.g. AVIF → MP4).
const OUTPUTS = [
  { attr: 'video-key',   filename: 'video.mp4',    mime: 'video/mp4',            label: 'video.mp4'    },
  { attr: 'preview-key', filename: 'preview.avif', mime: 'image/avif',           label: 'preview.avif',
    fallback: { filename: 'preview.mp4', mime: 'video/mp4' } },
...
class AfPictureInPicture extends HTMLElement {
  static observedAttributes = ['url', 'cache-url', 'video-key', 'preview-key', 'text-key',
  ...
  attributeChangedCallback(attr, prev, next) {
    if (prev === next) return;
    this.#applyAttrs();
    // Clear cached settings when server URL or job type could change
    this.#settings = null;
...
customElements.define('af-picture-in-picture', AfPictureInPicture);
export { AfPictureInPicture };
