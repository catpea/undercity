...
function regenerateCategoryIndex(catDir, catId, outPath) {
  ...
  // Dynamically fetch category.json and each action.json at module load time.
  // This way edits to action.json are picked up immediately without regenerating index.js.
  ...
  const fetchLines = [
    ...
    ...actions.map(n =>
      `const ${n}Meta = await fetch(new URL('./${n}/action.json', import.meta.url)).then(r => r.json());`
    ),
  ].join('\n');

  const registrations = actions.map(n =>
    `      [${n}Meta.id]: { ...${n}Meta, run: ${n}Run },`
  ).join('\n');
