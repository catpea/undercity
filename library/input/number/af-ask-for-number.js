...
class AfAskForNumber extends HTMLElement {
  static observedAttributes = ['key', 'label', 'placeholder', 'min', 'max', 'step', 'required', 'help', 'size', 'valid-feedback', 'invalid-feedback'];
  ...
  connectedCallback() {
    ...
    const combined = Signal.combineLatest([
      this.#key, this.#label, this.#placeholder, this.#min, this.#max, this.#step,
      this.#required, this.#help, this.#size, this.#validFeedback, this.#invalidFeedback,
    ]);

    this.#scope.add(combined);
    this.#scope.add(combined.subscribe(([key, label, placeholder, min, max, step, required, help, size, validFeedback, invalidFeedback]) => {
      const fieldId = safeFieldId('af-number', key || 'field');
