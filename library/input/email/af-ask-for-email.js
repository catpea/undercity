// library/input/email/af-ask-for-email.js
//
// <af-ask-for-email key="email" label="Email Address" required>
//
// Observed attributes:
...
template.innerHTML = `
  ...
  <input part="input" type="email" inputmode="email" autocomplete="email" spellcheck="false">
...
class AfAskForEmail extends HTMLElement {
  static observedAttributes = ['key', 'label', 'placeholder', 'required', 'help', 'size', 'valid-feedback', 'invalid-feedback'];
  ...
  connectedCallback() {
    ...
    this.#scope.add(combined.subscribe(([key, label, placeholder, required, help, size, validFeedback, invalidFeedback]) => {
      const fieldId = safeFieldId('af-email', key || 'field');
      ...
      this.#inputEl.placeholder = placeholder || 'you@example.com';
...
customElements.define('af-ask-for-email', AfAskForEmail);
export { AfAskForEmail };
