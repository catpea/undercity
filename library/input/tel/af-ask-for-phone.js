...
class AfAskForPhone extends HTMLElement {
  static observedAttributes = ['key', 'label', 'placeholder', 'required', 'pattern', 'help', 'size', 'valid-feedback', 'invalid-feedback'];
  ...
  connectedCallback() {
    ...
    this.#scope.add(combined.subscribe(([key, label, placeholder, required, pattern, help, size, validFeedback, invalidFeedback]) => {
      const fieldId = safeFieldId('af-tel', key || 'field');
      ...
      setOptionalAttribute(this.#inputEl, 'pattern', pattern);
