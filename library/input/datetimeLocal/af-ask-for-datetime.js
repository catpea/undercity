// library/input/datetimeLocal/af-ask-for-datetime.js
//
// <af-ask-for-datetime key="scheduledAt" label="Scheduled At">
//
// Observed attributes:
...
template.innerHTML = `
  ...
  <input part="input" type="datetime-local">
...
class AfAskForDatetime extends HTMLElement {
  static observedAttributes = ['key', 'label', 'min', 'max', 'required', 'help', 'size', 'valid-feedback', 'invalid-feedback'];
  ...
  connectedCallback() {
    ...
    const combined = Signal.combineLatest([
      this.#key, this.#label, this.#min, this.#max, this.#required,
      this.#help, this.#size, this.#validFeedback, this.#invalidFeedback,
    ]);

    this.#scope.add(combined);
    this.#scope.add(combined.subscribe(([key, label, min, max, required, help, size, validFeedback, invalidFeedback]) => {
      const fieldId = safeFieldId('af-datetime', key || 'field');
      ...
      setOptionalAttribute(this.#inputEl, 'min', min);
      setOptionalAttribute(this.#inputEl, 'max', max);
...
customElements.define('af-ask-for-datetime', AfAskForDatetime);
export { AfAskForDatetime };
