// library/input/url/af-ask-for-url.js
//
// <af-ask-for-url key="website" label="Website" placeholder="https://example.com">
//
// Observed attributes:
...
//
// Input type: url, inputmode="url", autocomplete="url", spellcheck=false.
...
template.innerHTML = `
  ...
  <input part="input" type="url" inputmode="url" autocomplete="url" spellcheck="false">
  <div part="error" class="error"></div>
`;

class AfAskForUrl extends HTMLElement {
  static observedAttributes = ['key', 'label', 'placeholder', 'required'];
  ...
  connectedCallback() {
    const combined = Signal.combineLatest([this.#key, this.#label, this.#placeholder, this.#required]);
    this.#scope.add(combined);
    this.#scope.add(combined.subscribe(([key, label, placeholder, required]) => {
      const id = `af-url-${key || 'field'}`;
      this.#labelEl.textContent = label || key;
      this.#labelEl.setAttribute('for', id);
      this.#inputEl.id          = id;
      this.#inputEl.name        = key;
      this.#inputEl.placeholder = placeholder || 'https://example.com';
...
customElements.define('af-ask-for-url', AfAskForUrl);
export { AfAskForUrl };
